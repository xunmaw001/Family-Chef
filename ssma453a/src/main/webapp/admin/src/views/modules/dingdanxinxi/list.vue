<template>
  <div class="main-content">
      ...
      <div class="table-content">
        <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
            ...
            <el-table-column width="300" :align="contents.tableAlign"
                ...
                <el-button v-if="isAuth('dingdanxinxi','用户评价') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 1" type="success" icon="el-icon-tickets" size="mini" @click="yonghupingjiaCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'用户评价':'' }}</el-button>
                <el-button v-if="isAuth('dingdanxinxi','用户评价') && contents.tableBtnIcon == 1 && contents.tableBtnIconPosition == 2" type="success" size="mini" @click="yonghupingjiaCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'用户评价':'' }}<i class="el-icon-tickets el-icon--right" /></el-button>
                <el-button v-if="isAuth('dingdanxinxi','用户评价') && contents.tableBtnIcon == 0" type="success" size="mini" @click="yonghupingjiaCrossAddOrUpdateHandler(scope.row,'cross')">{{ contents.tableBtnFont == 1?'用户评价':'' }}</el-button>
    ...
    <yonghupingjia-cross-add-or-update v-if="yonghupingjiaCrossAddOrUpdateFlag" :parent="this" ref="yonghupingjiaCrossaddOrUpdate"></yonghupingjia-cross-add-or-update>
...
import yonghupingjiaCrossAddOrUpdate from "../yonghupingjia/add-or-update";
export default {
  data() {
    return {
      searchForm: {
      ...
      yonghupingjiaCrossAddOrUpdateFlag: false,
  ...
  methods: {
    ...
    yonghupingjiaCrossAddOrUpdateHandler(row,type){
      this.showFlag = false;
      this.addOrUpdateFlag = false;
      this.yonghupingjiaCrossAddOrUpdateFlag = true;
      this.$storage.set('crossObj',row);
      this.$storage.set('crossTable','dingdanxinxi');
      this.$nextTick(() => {
      this.$refs.yonghupingjiaCrossaddOrUpdate.init(row.id,type);
      });
    },
    init () {
